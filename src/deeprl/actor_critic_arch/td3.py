...
class TD3:
    ...
    def _update(self) -> None:
        ...
        TD_targets = batch.rewards + ~batch.terminateds * self._discount_factor * torch.min(*[target_critic(batch.next_states, target_actions) for target_critic in self._target_critics])
        ls_action_values = [critic(batch.states, batch.actions) for critic in self._critics]

        critic_loss = torch.add(*[F.mse_loss(TD_targets, action_values) for action_values in ls_action_values])
